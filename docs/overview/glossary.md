...
| ChromaDB | OSSのベクトルデータベース。埋め込みベクトルの保存・検索に使用。API v2を使用 |
...
| STRICT | IstioのmTLSモード。非暗号化通信を拒否する最も厳しいモード |
| ArgoCD | GitOpsベースのCDツール。Gitリポジトリの変更を自動デプロイ |
| GitOps | Gitリポジトリを単一の真実のソースとして使用するCI/CDの手法 |
| Self-hosted Runner | GitHub Actionsをローカル環境（Mac M1）で実行するためのエージェント |
