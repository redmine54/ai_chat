...
本ドキュメントは、k8s / IaC / GitLabを活用したマイクロサービス開発環境の設計方針を定めるものです。
CI/CD・RBAC・セキュリティ・ITGC対応・FinOpsの観点から、compose・minikube・staging・productの4段階環境での開発・試験・運用フローを規定します。
...
| 項目 | 採用技術・方針 |
|---|---|
| インフラ | Kubernetes / AKS / Terraform（IaC） |
| CI/CD | GitLab CI/CD |
| 認可管理 | RBAC（役割ベースアクセス制御） |
| 構成管理 | GitLab（社内LAN接続） |
| プロジェクト管理 | Redmine |
| ネットワーク | 社内LAN + Azure閉域網 |
| 外部公開 | ngrok（compose環境・単体試験時のユーザーレビューのみ） |
...
| minikube環境 | Minikube / ローカルPC | composeでは検証できないk8s機能の試験 |
| staging環境 | AKS（Azure） | 他機能との組合せ試験・ITGC確認 |
| product環境 | AKS（Azure） | 総合試験・本番運用・ITGC確認 |

### セキュリティ基本原則

| 原則 | 内容 |
|---|---|
| Secrets分離 | パスワード・APIキーは環境ごとに完全分離 |
| DB分離 | 環境ごとにDBを分離し流用禁止 |
| GitLab登録禁止 | Secrets・本番データのGitLab登録禁止 |
| 本番データ禁止 | 開発・試験環境での本番データ使用禁止 |
...
- **Secrets管理**：CI変数（`CI/CD → Variables`）でAPIキーを管理し、Masked設定必須

### minikube環境

- **ブランチ**：`develop` ブランチのCI上でk8sマニフェスト検証（`kubectl apply --dry-run`）を追加
- **マニフェスト管理**：`k8s/overlays/minikube` をGitLabで管理
- **Secrets管理**：k8s SecretはマニフェストにはせずCI変数から `kubectl create secret` で投入

### staging環境

- **ブランチ**：`develop` → `staging` へのマージをトリガーにデプロイ
- **承認フロー**：stagingへのMRはリードエンジニア承認必須（Protected Branch + Approvals設定）
- **ArgoCD連携**：GitLab CIがAKSのArgoCDに同期指示し自動デプロイ
- **Secrets管理**：Azure Key Vault + External Secrets OperatorでSecretを注入（GitLabには保存しない）
...
- **監査ログ**：GitLab Audit Eventsを有効化し全操作を記録
...
- staging と product は **別VNet** で分離し、相互ピアリングも禁止
- GitLab RunnerはAzureへPrivate Link経由でのみ接続
...
| 定期アクセスレビュー | 四半期ごとにGitLab / Azure IAMの権限棚卸しを実施 |

### ② 変更管理（Program Change）

| 対応事項 | 実装方法 |
|---|---|
| 変更申請 | Redmineチケットで変更内容・影響範囲・テスト結果を記録 |
| コードレビュー | GitLab MRで必須レビュー・承認（stagingは1名・productは2名） |
| テスト証跡 | GitLab CIのパイプライン実行結果をテスト証跡として保持 |
...
| IaC管理 | TerraformでInfrastructureをコード管理し変更履歴を保持 |
